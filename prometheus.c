...
char    *log_file_mode = "w";   /* overwrite command and iptables log files, except append in -x mode */
...
struct Textfile *previous_commands = NULL, *previous_iptables = NULL, *previous_ip6tables = NULL, *previous_classmap = NULL, *textline;
...
void safe_run(char *cmd)
{
 ...
 if(log_file && !mix_new_hosts)//!!debug
 {
  fprintf(log_file,"%s\n",cmd);
 }
}
...
Credit: CZFree.Net, Martin Devera, Netdave, Aquarius, Gandalf\n\n",version);
 ...
 arguments
 {
  ...
  argument("-x") { run = TRUE; mix_new_hosts = TRUE; log_file_mode = "a"; }
  argument("-r") { run = TRUE; }
...
*/
 ...
 /*-----------------------------------------------------------------*/
  ...
  /*-----------------------------------------------------------------*/
  load(textline, previous_classmap, classmap, Textfile, str);
 }
 ...
 if(!just_preview)
 {
  ...
  log_file = fopen(cmdlog, log_file_mode); /* overwrite command and iptables log files, except append in -x mode */
  if(log_file == NULL) 
  {
    perror(cmdlog);
    exit(-1);
  ...
  if(!mix_new_hosts)
  {  
   iptables_file = fopen(iptablesfile, log_file_mode); /* overwrite command and iptables log files, except append in -x mode */
   iptables_save_line(iptablespreamble, IPv4);
   if(ip6prefix)
   {
    ip6tables_file = fopen(ip6tablesfile, log_file_mode); /* overwrite command and iptables log files, except append in -x mode */
    iptables_save_line(iptablespreamble, IPv6);
    iptables_save_line(ip6preamble, IPv6);
   }
  }
